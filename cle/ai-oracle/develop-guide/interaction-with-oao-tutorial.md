...
At the end we will deploy the contract to Optimism mainnet and interact with it.
...
### Import dependencies
At the beginning we need to import several dependencies which our smart smart contract will use.
```javascript
...
Now let’s define the mapping that keeps track of the callback gas limit for each model and set the initial values inside the constructor. We’ll also define a modifier so that only the contract owner can change these values.
...
We want to store all the requests that happened, so we create a data structure that contains request data and the mapping between requestId and the request data.
...
```
We assigned prompt, sender and the modelId to the request and also emitted an event. <br/>
...
```
We've overriden the callback function from the `AIOracleCallbackReceiver.sol`. It's important to put the modifier, so that only OAO can callback into our contract. <br/>
First we check if the request with provided id exists. If it does we assign the output to the request. <br/>
...
At the end let's add the method that will estimate the fee for the callback call.
```javascript
...
2. Open your solidity development environtment. In this case we'll use [Remix IDE](https://remix.ethereum.org/).
...
5.  Deploy the compiled bytecode
    ...
    2. To deploy our prompt contract we need to provide the address of AIOracle contract. This contract is already deployed on the blockchain. You can find this address on the [reference page](https://docs.ora.io/doc/cle/ai-oracle/reference). We are looking for OAO_PROXY address.
        <div>
            <img src="../../../.gitbook/assets/remix_deployment.png" alt="" width="300px" style="margin-top: 10px;"/>
        </div>
    3. Deploy the contract by signing transaction in the wallet.
 6. Once the contract is deployed, you can interact with it. 
    ...
    Let's use Stable Diffusion model (id = 50). Call estimate fee method to calculate fee for the callback.
    <div>
        <img src="../../../.gitbook/assets/estimateFee_remix.png" alt="" width="300px" style="margin-top: 10px;"/>
    </div>
    Then request the result from OAO by calling calculateAIResult method. Pass the model id and the prompt for the image generation. Remember to pass estimated fee as a value for the transaction.
    <div>
        <img src="../../../.gitbook/assets/calculateAIResult_remix.png" alt="" width="300px" style="margin-top: 10px;"/>
    </div>
    After the transaction is executed, and the OAO calculates result, you can check it by calling prompts method. Simply input model id and the prompt you used for image generation.
    <div>
        <img src="../../../.gitbook/assets/calculateAIResult_remix.png" alt="" width="300px" style="margin-top: 10px;"/>
    </div>

    ## Conclusion
    In this tutorial we covered step by step writing of a solidity smart contract that interacts with ORA's On-chain AI Oracle. Then we compiled and deployed our contract to the live network and interacted with it. In the next tutorial, we will extend the functionality of Prompt contract to support AI generated NFT collections.
